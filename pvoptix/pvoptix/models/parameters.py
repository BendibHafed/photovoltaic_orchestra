...
Implements equations (4-10) from NCMAI'26 paper:
- Equation (4): Iph(G,T)
- Equations (5-6): I01(T), I02(T)
- Equation (9): Rsh(G)
- Equation (10): Rs(T) (constant in this work)
"""
...
def iph_model(G: float, T: float, Iph_stc: float, alpha_I: float = None) -> float:
    ...
    Args:
        ...
        alpha_I: Temperature coefficient [1/K] (default from default_coeffs)
...
def i0_model(T: float, I0_stc: float, n: float) -> float:
    """
    Temperature-dependent diode saturation current model (single diode).

    Used for legacy single-diode model.

    Args:
        T: Temperature [K]
        I0_stc: Saturation current at STC [A]
        n: Ideality factor

    Returns:
        Saturation current at temperature T [A]
    """
    exponent = (Eg / (n * k)) * (1.0 / T_STC - 1.0 / T)
    exponent = np.clip(exponent, -100, 100)
    return I0_stc * (T / T_STC) ** 3 * np.exp(exponent)


def i01_model(T: float, I01_stc: float, n1: float) -> float:
    """
    Temperature-dependent diode 1 saturation current (double diode).
...
def i02_model(T: float, I02_stc: float, n2: float) -> float:
    """
    Temperature-dependent diode 2 saturation current (double diode).
...
def rs_model(
    ...
    """
    Series resistance model with temperature and irradiance effects.

    Equation (10) from paper (constant in this work, but extended for flexibility).
    ...
    # Temperature effect: linear correction
    temp_effect = 1.0 + alpha_Rs * (T - T_STC)

    # Irradiance effect: linear correction
...
def rsh_model_double(G: float, Rsh_stc: float) -> float:
    ...
    Equation (9) from NCMAI'26 paper:
    ...
    """
    # Irradiance dependence (inverse)
    rsh = Rsh_stc * (G_STC / max(G, 1e-6))
    return max(rsh, 1.0)  # Clamp to avoid non-physical values