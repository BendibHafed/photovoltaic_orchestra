...
from pvoptix.pvoptix.models.double_diode import double_diode_equation
...
def solve_current_double(
    ...
    # Estimate Voc for bracketing (using dominant diode)
    try:
        a1 = n1 * Ns * Vt
        Voc_est = a1 * np.log(Iph / max(I01, 1e-12))
        Voc_est = min(Voc_est, 50.0)  # Sanity check
    except Exception:
